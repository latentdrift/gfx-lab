...
The document has explicit **Scene Defaults** followed by a top-to-bottom operation stack and **Final Output**. A **Render** operation applies inherited renderer state plus sparse local overrides and produces named Color, Depth, Field, and Spectrum16 resources. An **Interpret** operation converts one spectral resource through a selected observer without rerendering the scene. A **Composite** operation combines two named signals with explicit arithmetic. A terminal **Measure** consumer samples any upstream operation without changing the accumulated image and reports signed mean RGB, mean magnitude, RMS energy, peak magnitude, and coverage above an authored threshold. This makes a visual disagreement usable as quantitative data. The Inspector shows only controls owned by the selected object. Editing a local Render value creates an override, and returning it to the scene-default value restores inheritance.
