...
constexpr std::array articles = {
  ...
  Article{"Animation", "Keyframes and parameter animation",
    "A property track contains keyframes for one named parameter. Each key stores that property's value at one time. Evaluation finds the surrounding keys on each track and computes the in-between value using step, linear, or smooth-step interpolation. Graphics Lab evaluates sparse tracks into a temporary render-pass stack, leaving base pass values intact.",
    ...
    "A whole-pass snapshot hides which values are intentionally animated and couples unrelated edits. Independent tracks make key ownership, deletion, and interpolation explicit. Continuous numbers and colors interpolate naturally; algorithms, resources, topology, enable switches, and pass ordering require discrete transitions or another representation. Angle interpolation may need explicit wrap handling for large rotations.",
    "A dope sheet shows property tracks vertically and time horizontally; diamonds mark keys. State the property, units, key times, interpolation curve, looping behavior, and whether evaluation changes source data or produces a temporary frame state. A timeline animates parameters; skeletal animation separately deforms a mesh through a bone hierarchy.", Diagram::Animation},
