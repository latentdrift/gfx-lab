...
constexpr std::array articles = {
  ...
  Article{"Fields & implicit surfaces", "Persistent simulation fields",
    "A persistent field stores quantities over a spatial grid and evolves them through time. Unlike an analytic field, its current value depends on prior injections, transport, reactions, dissipation, and boundaries.",
    "Simulation update before rendering", "Heat, smoke, fuel, moisture, pressure, and velocity remain after their source changes and can travel through the domain.",
    "A stable simulation requires a fixed timestep, explicit units, boundary conditions, and separate storage for independent quantities.",
    "Distinguish field producers, simulation operators, persistent channels, obstacles, and consumers.", Diagram::RenderGraph},
  Article{"Fields & implicit surfaces", "Advection, combustion, and pressure projection",
    "Advection moves stored quantities through a velocity field. Combustion converts sufficiently hot fuel into heat and smoke. Pressure projection removes velocity divergence so flow respects an approximately incompressible medium and solid boundaries.",
    "Persistent elemental simulation", "A hot fuel source ignites, rises, curls around obstacles, produces smoke, and leaves state behind after injection stops.",
    "Grid resolution, timestep, interpolation, reaction rates, pressure iterations, dissipation, and obstacle sampling jointly determine stability and appearance.",
    "Ask for advection scheme, reaction model, pressure solve, boundary representation, grid resolution, and fixed simulation rate.", Diagram::RenderGraph},
  ...
  Article{"Color", "Cone, rod, and opponent-response approximations",
    ...
    "Observer transform at a composite operand or after the complete stack", "The same pass can feed A as cone luminance and B as rod response before difference, XOR, min/max, or other arithmetic. Downstream display reconstruction instead reinterprets the already completed stack.",
    "An RGB-to-LMS matrix approximates receptor response but cannot recover the original spectrum: metameric spectra that produced the same RGB remain identical. Operand XOR exposes quantization-threshold disagreement between measurements; it is expressive arithmetic, not a biological model.",
  ...
  Article{"Output", "Render algebra between completed images",
    "Render algebra compares correlated signals with a per-pixel, usually per-channel relationship F(A, B). Absolute difference preserves disagreement magnitude; signed difference also preserves which input produced the larger value. Explicit graph connections determine which representations are compared.",
    "A typed relationship operation after its two producers have generated compatible signals",
  ...
  Article{"Engine architecture", "Operation graphs and controlled perturbation",
    "An operation graph rerenders a shared scene several ways, derives working signals, and connects those signals to explicit consumers. Duplicating a render operation preserves spatial correlation while its property overrides state exactly what makes the new interpretation different.",
    "Dependency compilation, operation scheduling, typed signals, transient render targets, and fullscreen processing",
    ...
    "A duplicated framebuffer can only receive image-space edits; a duplicated render operation can alter the 3D interpretation and rerasterize it. Explicit connections expose branching and reuse, while the compiler rejects cycles and mismatched inputs. More renders still increase scene submission and fill cost, but transient signals can reuse storage after their last consumer.",
    "Name each operation by what it computes. Show its typed inputs, produced signals, property overrides, animation tracks, resolution, and estimated cost. Routine commands may create and connect several operations automatically, but the resulting graph remains visible and editable.", Diagram::Comparison},
  Article{"Animation", "Keyframes and parameter animation",
    "A typed property track contains keyframes for one named parameter on one stable operation ID. Continuous values may use step, linear, or smooth-step interpolation. Booleans, integers, and algorithm selections always step because values between their legal states have no meaning. Graphics Lab evaluates tracks into temporary operation state, leaving authored values intact.",
    "Animation timeline evaluation before graph execution",
    ...
    "A whole-operation snapshot hides which values are intentionally animated and couples unrelated edits. Independent typed tracks make key ownership, deletion, and legal interpolation explicit. Resource allocation and graph topology stay outside ordinary parameter animation because changing them can rebuild GPU storage or restructure the document. Angle interpolation may need explicit wrap handling for large rotations.",
    "A dope sheet shows operation property tracks vertically and time horizontally; diamonds mark keys. State the property type, units, key times, legal interpolation, looping behavior, and whether evaluation changes source data or produces a temporary frame state. A timeline animates operation parameters; skeletal animation separately deforms a mesh through a bone hierarchy.", Diagram::Animation},
...
constexpr std::array quickReads = {
  ...
  QuickRead{"Operation graphs and controlled perturbation", "Duplicate a render, change only what matters, then connect its signals wherever they produce useful disagreement."},
  QuickRead{"Keyframes and parameter animation", "Animate a named property on a stable operation without baking unrelated state into the track."},
...
std::array<const char*, 4> branchesFor(std::string_view title) {
  ...
  if (title == "Render algebra between completed images") return {"Operation graphs and controlled perturbation", "Transparency and compositing", "Linear light and encoded RGB", "Color quantization and dithering"};
  if (title == "Operation graphs and controlled perturbation") return {"Render algebra between completed images", "Keyframes and parameter animation", "Forward rendering and render passes", "Render graphs and pass dependencies"};
  if (title == "Keyframes and parameter animation") return {"Operation graphs and controlled perturbation", "Skeletal animation and skinning", "Morph targets and procedural deformation", "Render graphs and pass dependencies"};
  if (title == "Undo, redo, and editor transactions") return {"Operation graphs and controlled perturbation", "Keyframes and parameter animation", "Asset, scene, material, and renderer responsibilities", "Render graphs and pass dependencies"};
