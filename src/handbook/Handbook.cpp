...
constexpr std::array articles = {
  ...
  Article{"Animation", "Keyframes and parameter animation",
    "A typed property track contains keyframes for one named parameter. Continuous values may use step, linear, or smooth-step interpolation. Booleans, integers, and algorithm selections always step because values between their legal states have no meaning. Graphics Lab evaluates sparse tracks into a temporary render-pass stack, leaving base pass values intact.",
    ...
    "A whole-pass snapshot hides which values are intentionally animated and couples unrelated edits. Independent typed tracks make key ownership, deletion, and legal interpolation explicit. Resource allocation, topology, pass creation, and pass ordering are kept outside ordinary parameter animation because changing them can rebuild GPU storage or restructure the document. Angle interpolation may need explicit wrap handling for large rotations.",
    "A dope sheet shows property tracks vertically and time horizontally; diamonds mark keys. A pass-difference audit answers a separate question: which base values or tracks diverged after duplication? State the property type, units, key times, legal interpolation, looping behavior, and whether evaluation changes source data or produces a temporary frame state. A timeline animates parameters; skeletal animation separately deforms a mesh through a bone hierarchy.", Diagram::Animation},
...
constexpr std::array quickReads = {
  ...
  QuickRead{"Keyframes and parameter animation", "Give each animated property its own row of timed values, then evaluate every row at the playhead before rendering the frame."},
